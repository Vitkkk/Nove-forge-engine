...
class NovaViewportView(context: Context) : View(context) {
    ...
    private val scaleDetector = ScaleGestureDetector(context, object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
        override fun onScale(detector: ScaleGestureDetector): Boolean {
            val beforeX = (detector.focusX - panX) / zoom
            val beforeY = (detector.focusY - panY) / zoom
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        runtime?.let { running ->
            val worldX = (event.x - panX) / zoom
            val worldY = (event.y - panY) / zoom
        ...
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                selectedNode?.let { dragStartNodeX = it.transform.x; dragStartNodeY = it.transform.y }
                invalidate()
            }
            MotionEvent.ACTION_MOVE -> {
                ...
                if (panning || event.pointerCount > 1) {
                    ...
                    selectedNode?.let { node ->
                        var nx = node.transform.x + dx / zoom
                        var ny = node.transform.y + dy / zoom
                        if (snapEnabled && abs(dx) + abs(dy) > 2f) {
                            nx = (nx / grid).toInt() * grid
                            ny = (ny / grid).toInt() * grid
                        }
                        node.transform.x = nx; node.transform.y = ny
                    }
                }
                lastX = event.x; lastY = event.y
                invalidate()
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                selectedNode?.takeIf { draggingNode }?.let { node ->
                ...
                draggingNode = false; panning = false
            }
        }
        return true
    }
